...
- [ ] 메뉴(2차원 배열)
  - [ ] 배열(혹은 객체) 안에 들어있는 모든 수의 합이 20개를 넘는 지 확인
  - [ ] 배열(혹은 객체) 안에 들어있는 모든 메뉴의 가격 합이 10,000원을 넘는 지 확인
  - [ ] 배열 내부에 중복되는 항목이 있는지 확인
...
- [ ] 메뉴(2차원 배열)
  ...
  - [ ] 배열 안에 들어있는 모든 메뉴의 가격 합이 10,000원을 넘는 지 확인
  - [ ] 배열 내부에 중복되는 항목이 있는지 확인
...
- [x] 메뉴(입력받은 메뉴 문자열)
  - [x] 입력받은 값이 존재하는지 확인
  - [x] `,`를 기준으로 나누었을 때 `-`가 들어있는 지 확인
- [ ] 메뉴(문자열을 `,` 기준으로 나누고 `-`기준으로 한번 더 나누었을 때 나오는 배열의 0번째 자리의 문자열)
  - [ ] 입력받은 값이 존재하는지 확인
  - [ ] 실제 존재하는 메뉴이름인지 확인
- [ ] 메뉴(문자열을 `,` 기준으로 나누고 `-`기준으로 한번 더 나누었을 때 나오는 배열의 1번째 자리의 숫자)
  - [ ] 입력받은 값이 존재하는지 확인
  - [ ] 입력받은 날짜를 숫자로 변환했을 때 Number 형식인 지 확인
  - [ ] 숫자가 최솟값(1)보다 작은 지 확인
  - [ ] 숫자가 최댓값(2)보다 큰 지 확인
- [ ] 메뉴(2차원 배열)
